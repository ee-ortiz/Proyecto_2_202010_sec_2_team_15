...
public class MinPQ <T extends Comparable<T>>   {
	 ...
	 * @throws NoSuchElementException if this priority queue is empty
	 */
	public T delMin(Comparator<Comparendo> pComparador) {
		if (isEmpty()) throw new NoSuchElementException("Priority queue underflow");
		T min = pq[1];
		exch(1, n--);
		sink(1, pComparador);
	...
	private void swim(int k, Comparator<Comparendo> pComparador) 
	{
		if(pComparador==null){
			...
			while (k > 1 && greater(k/2, k)&& pComparador.compare((Comparendo)pq[k/2], (Comparendo)pq[k])>0) {
				exch(k, k/2);
				k = k/2;
			}
		}
	...
	public Iterator<T> iterator(Comparator<Comparendo> pComparador) {
		return (Iterator<T>) new HeapIterator(pComparador);
	}
	...
	private class HeapIterator implements Iterator<T> {
		...
		public HeapIterator(Comparator<Comparendo> pComparador) {
			if (comparator == null) copy = new MinPQ<T>(size());
			else                    copy = new MinPQ<T>(size(), comparator);
			for (int i = 1; i <= n; i++)
				copy.insert(pq[i], pComparador);
		}
		...
		public T next() {
			...
			return copy.delMin(this.pComparador);
		}
	}
